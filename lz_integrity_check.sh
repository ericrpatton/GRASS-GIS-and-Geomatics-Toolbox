...
############################################################################
#
# MODULE:        lz_integrity_check.sh
# 
# AUTHOR:   	 Eric Patton, Geological Survey of Canada (Atlantic)
# 		         <Eric dot Patton at Canada dot ca>
#
# PURPOSE:       To verify the integrity and caclulate the SHA256 cryptographic
# hash of lzip-ed files in the current directory, and write these results to
...
if [ "$1" == "-H" -o "$1" == "-h" -o "$1" == "--help" -o "$1" == "-help" ] ; then
	echo -e "\nusage: $SCRIPT: verifies the integrity and calculates the SHA256 cryptographic hash of lzip-ed files in the current directory, and writes these results to the screen and a report.  \n"
	exit 0
fi
...
FILE_COUNT=$(ls -1 *.lz 2>/dev/null | wc -l)

[[ ${FILE_COUNT} -eq 0 ]] && echo -e "\n$SCRIPT: Error: No lzip files in the current directory!" && exit 1
...
echo -e "Version: lzip Version 1.0.8, 13-Jul-2019" | tee -a integrity_check_and_sha256sum_report.txt
...
parallel -j16 lzip -tv 2>&1 ::: *.lz | tee -a tmp_check.txt
...
parallel -j16 sha256sum 2>&1 ::: *.lz | tee -a tmp_sha.txt && echo -e "\nDone."
